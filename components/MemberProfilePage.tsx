...
import { NavigationPage, User as AppUser } from "../src/App";
...
export function MemberProfilePage({
  ...
  const memberData: Record<string, Member> = {
    "1": {
      ...
      experience:
        "10+ years playing experience, Former captain of university team",
      certifications: [
  ...
  const getStatusBadge = (
    ...
    const variants = {
      ...
      Expired: {
        ...
        icon: <CalendarX className="w-3 h-3 mr-1" />,
      },
      Released: {
  ...
  const getAttachmentStatusBadge = (
    status: "verified" | "pending" | "rejected"
  ) => {
    const variants = {
      verified: {
  ...
  const getMatchResultBadge = (result: "Win" | "Loss" | "Draw") => {
    ...
    return <Badge className={`rounded-lg ${className}`}>{result}</Badge>;
  };
  ...
  const handleReleaseMember = () => {
    ...
    alert(
      `${member?.name} has been successfully released from ${member?.club}. They can now join another club or be tagged by other clubs.`
    );
          ...
          <div>
            ...
            <p className="text-gray-600">
              {member.status === "Pending"
                ? "Review pending membership application"
      ...
      <Dialog open={isReleaseDialogOpen} onOpenChange={setIsReleaseDialogOpen}>
        <DialogContent className="rounded-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to release <strong>{member.name}</strong>{" "}
              from <strong>{member.club}</strong>?
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
                ...
                <div className="flex flex-wrap justify-between items-center gap-2">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-600">
                      {member.seasonStats.matchesPlayed}
                    </div>
                    ...
                    <>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">
                          {member.seasonStats.wins}
                        </div>
                        <div className="text-xs text-gray-600">Wins</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-yellow-600">
                          {member.seasonStats.draws}
                        </div>
                        <div className="text-xs text-gray-600">Draws</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-red-600">
                          {member.seasonStats.losses}
                        </div>
                    ...
                    <>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-purple-600">
                          {member.seasonStats.goals}
                        </div>
                        <div className="text-xs text-gray-600">Goals</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-orange-600">
                          {member.seasonStats.assists}
                        </div>
                        <div className="text-xs text-gray-600">Assists</div>
                      </div>
                      {(member.position.toLowerCase().includes("keeper") ||
                        member.position.toLowerCase().includes("defender")) && (
                        <div className="text-center">
                          <div className="text-2xl font-bold text-green-700">
                            {member.seasonStats.cleanSheets}
                          </div>
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-yellow-500">
                      {member.seasonStats.yellowCards}
                    </div>
                            ...
                            attachment.status === "pending" && (
                              <div className="flex space-x-1">
                                <Button
                                  size="sm"
                                  onClick={() =>
                                    handleVerifyDocument(attachment.id)
                                  }
                                  className="bg-green-600 hover:bg-green-700 text-white rounded-lg"
                                >
                                ...
                                <Button
                                  ...
                                  variant="destructive"
                                  onClick={() =>
                                    handleRejectDocument(attachment.id)
                                  }
                                  className="rounded-lg"
                                >
