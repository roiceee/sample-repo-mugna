...
export function GalleryPage({ user, onNavigate }: GalleryPageProps) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            Club Owners don't have access to the Photo & Video Gallery.
          </p>
          <Button
  ...
  const [mediaItems, setMediaItems] = useState<MediaItem[]>([
    ...
    },
    {
      ...
      type: "image",
      thumbnail:
        "https://images.unsplash.com/photo-1577223625816-7546f13df37d?w=400&h=300&fit=crop",
      event: "Youth Championship Finals",
      category: "Tournament",
      date: "2025-06-20",
      views: 892,
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = files.filter((file) => {
      ...
      if (file.size > maxSize) {
        alert(
          `${file.name} is too large. Maximum size: ${file.type.startsWith("video/") ? "100MB for videos" : "25MB for images"}`
        );
        return false;
      }
      ...
      if (!allowedTypes.includes(file.type)) {
        alert(
          `${file.name} is not a supported file type. Supported: JPEG, PNG, GIF, WebP, MP4, WebM, MOV, AVI`
        );
        return false;
      }
  ...
  const handleUploadSubmit = () => {
    if (selectedFiles.length > 0 && uploadForm.title && uploadForm.category) {
      ...
      alert(
        `Successfully uploaded ${selectedFiles.length} file(s)! They are now visible in the gallery.`
      );
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
          ...
          >
            ...
            <DialogContent className="max-w-lg">
              <DialogHeader>
                ...
                <DialogDescription>
                  Share your photos and videos with the DSRFA community.
                  Supported formats: JPEG, PNG, GIF, WebP, MP4, WebM, MOV, AVI
                </DialogDescription>
              </DialogHeader>
                ...
                {selectedFiles.length > 0 &&
                  ...
                  uploadForm.category && (
                    <Alert>
                      ...
                      <AlertDescription>
                        Ready to upload {selectedFiles.length} file(s). Your
                        content will be immediately visible to all members.
                      </AlertDescription>
                    </Alert>
                  )}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  className="rounded-lg justify-start text-left font-normal"
                >
                  <CalendarIcon className="mr-2 h-4 w-4" />
                  {selectedDateFrom
                    ? selectedDateFrom.toDateString()
                    : "From Date"}
                </Button>
              </PopoverTrigger>
          ...
          {(selectedDateFrom || selectedDateTo) && (
            <div className="mt-4 flex items-center space-x-2">
              <Badge variant="secondary" className="rounded-lg">
                ...
                {selectedDateFrom
                  ? selectedDateFrom.toLocaleDateString()
                  : "Any"}{" "}
                - {selectedDateTo ? selectedDateTo.toLocaleDateString() : "Any"}
                <Button
                  variant="ghost"
                  size="sm"
      ...
      <div className="mb-4">
        <p className="text-gray-600">
          ...
          {(selectedDateFrom || selectedDateTo) && (
            <span className="ml-2">
              • Filtered by date:{" "}
              {selectedDateFrom ? selectedDateFrom.toLocaleDateString() : "Any"}{" "}
              - {selectedDateTo ? selectedDateTo.toLocaleDateString() : "Any"}
            </span>
          )}
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredItems.map((item) => (
          <Card
            key={item.id}
            className="group overflow-hidden hover:shadow-lg transition-shadow cursor-pointer rounded-lg"
          >
